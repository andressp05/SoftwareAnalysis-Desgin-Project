...
public class Ejercicio extends Recurso implements Serializable {
	...
	public Ejercicio(String titulo, LocalDate fechaInicio, LocalDate fechaFin, Double ponderacion, boolean aleatorio,
			String comentarios, Asignatura asig) {
	...
	public double getNotaMedia() throws IOException {
		if (notaMedia == -1) {
	...
	public double getpAciertos() throws IOException {
		if (pAciertos == -1) {
			setPorcentajes();
		}
		return pAciertos;
	...
	public double getpFallos() throws IOException {
		if (pFallos == -1) {
			setPorcentajes();
		}
		return pFallos;
	...
	public double getpBlancos() throws IOException {
		if (pBlancos == -1) {
			setPorcentajes();
		}
		return pBlancos;
	 ...
	 */
	public boolean anyadirPregunta(double peso, String enunciado, double penalizacion, TipoRespuesta tipoRespuesta) {
		Pregunta pregunta = new Pregunta(cont, peso, enunciado, penalizacion, tipoRespuesta);
		aumentarCont();
	 ...
	 * @throws IOException 
	 */
	public double calcularNotaMedia() throws IOException {
		double suma = 0;
		int cont = 0;
	...
	public void setPorcentajes() throws IOException {
	...
	public String toString() {
		return "Ejercicio: " + this.getTitulo();
	}
	...
	/**
	 * Comprueba si un ejercicio ha sido contestado ya por el usuario actual
	 * @return true si ya contesto, false en caso contrario
	 */
	public boolean isContestado() {
		try {
			Alumno alum = ((Alumno)Aplicacion.getInstance().getUsuarioAct());
			if(alum.getEjercicioAlumnoPorEjercicio(this) != null) {
				return true;
			}
			return false;
		} catch (IOException e) {
